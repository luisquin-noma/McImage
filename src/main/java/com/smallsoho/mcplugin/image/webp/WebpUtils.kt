...
class WebpUtils {
    ...
    companion object {
        ...
        private fun formatWebp(imgFile: File, outputDir: File) {
            if (ImageUtil.isImage(imgFile)) {
                val webpFile = File(outputDir, "${imgFile.name.substring(0, imgFile.name.lastIndexOf("."))}.webp")
        ...
        fun securityFormatWebp(imgFile: File, config: Config, project: Project, outputDir: File) {
                ...
                if(config.isSupportAlphaWebp) {
                    ...
                    if(imgFile.name.endsWith(Const.JPG) || imgFile.name.endsWith(Const.JPEG)) {
                        ...
                        if(!ImageUtil.isAlphaPNG(imgFile)) {
                            ...
                            //包含透明通道的png，进行压缩
                            CompressUtil.compressImg(imgFile, outputDir)
                        }
                    }
                }
            }
